...
async function showTournamentPage() {
    ...
    async function askPlayerCount() {
        ...
        ctx.fillText(translate('howManyPlayer'), canvas.width / 2, canvas.height / 2.3);
    ...
    async function getPlayerNames(playerCount, currentPlayerIndex, players) {
        ...
        input.type = 'text';
        input.placeholder = translate('namePLayer') + ` ${currentPlayerIndex}`;
        ...
        input.onkeydown = function(event) {
            if (event.key === 'Enter') {
                let playerName = input.value.replace(/[^ -~]/g, '').substring(0, 18);
                if (!playerName) {
    ...
    function showCustomizationOptions(players, playerCount) {
        ...
        ctx.fillStyle = "white";
        ctx.textAlign = "center";
        ctx.fillText(translate('chooseGameMode'), canvas.width / 2, canvas.height / 2.3);
    ...
    async function showTournament(players, playerCount) {
		...
		async function nextMatch() {
			if (currentMatch >= roundMatches) {
				...
				if (roundMatches < 1) {  // Check if roundMatches is less than 1 to end the tournament
					...
					ctx.fillText(translate('winnerTournament'), canvas.width / 2, canvas.height / 3);
					ctx.fillText(`🏆🏆   ${players[0]} !  🏆🏆`, canvas.width / 2, canvas.height / 2.4);
					ctx.fillText(translate('congrat'), canvas.width / 2, canvas.height / 2);
					ctx.font = "20px Arial";
        ...
        let part = translate('tournamentStart') + "\n" + "\n";
    ...
    function showPongTour(player1Name, player2Name, handleWinner) {
        ...
        function drawScore() {
            ...
            ctx.fillStyle = "white";
            ctx.font = "32px Arial";
            let player1Text = `${player1Name} : ${player1.score}`;
            let player2Text = `${player2Name} : ${player2.score}`;

            let player2TextWidth = ctx.measureText(player2Text).width;

            // Positionner le texte du joueur 1
            ctx.fillText(player1Text, 20, 50);

            // Ajuster la position du texte du joueur 2 pour qu'il reste dans les limites du canvas
            let player2XPosition = canvas.width - player2TextWidth - 20;
            ctx.fillText(player2Text, player2XPosition, 50);
        }
